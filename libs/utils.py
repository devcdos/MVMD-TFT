...
def tensorflow_quantile_loss(y, y_pred, quantile):
  ...
  q_loss = quantile * tf.maximum(prediction_underflow, 0.) + (
      1. - quantile) * tf.maximum(-prediction_underflow, 0.)#loss改掉试试

  # q1=quantile * tf.maximum(e1, 0.) + (
  #         1. - quantile) * tf.maximum(-e1 0.)

  # e1=tf.maximum(prediction_underflow, 0.).eval()
  # q=q_loss.eval()
  # s=tf.reduce_sum(q_loss,axis=-1).eval()
  # print(s)
  print("yshape",y.shape.as_list())
  print("ypredshape",y_pred.shape.as_list())
  print("q_lossshape",q_loss.shape.as_list())
...
#???
  ...
  normaliser = y.abs().sum()
  # quantile_loss = weighted_errors.mean()
  # normaliser = y.abs().mean()

  return 2 * quantile_loss / normaliser ,np.maximum(prediction_underflow, 0.),np.maximum(-prediction_underflow, 0.),normaliser
...
def add_copy_col(new_col,data,origin_col):
      col=data[[origin_col]].copy()
      col.columns=[new_col]
      return pd.concat([data,col],axis=1)
...
def individualforecast(model,test,weight_path,VMD_flag=False):
  if weight_path:
    model.load(weight_path)
  data=model.get_batch_data(test)
  input=data['inputs']
  output=data['outputs']
  combined={'predictp50':{},'actual':{},
            'loss':{'mse':{},
                    'mae':{},
                    'rsquared':{}},
            'predictp90': {
  ...
  b1list=[]
  group_by=test.groupby('id')
  for i,df in enumerate(group_by):
    b1list.append(df[0])
    #single_len=192
    #single_len=217
    single_len = input.shape[0]//len(group_by)
    # l=len(output[single_len * i:single_len * (i + 1), Ellipsis])
    # x=input[single_len*i:single_len*(i+1),Ellipsis]
    if VMD_flag:
        combined['predictp50'][df[0]] = model.model.predict(input[single_len * i:single_len * (i + 1), Ellipsis])[
        Ellipsis, 1]
        combined['predictp90'][df[0]] = model.model.predict(input[single_len * i:single_len * (i + 1), Ellipsis])[
            Ellipsis, 2]
        combined['predictp10'][df[0]] = model.model.predict(input[single_len * i:single_len * (i + 1), Ellipsis])[
            Ellipsis, 0]
        combined['actual'][df[0]] = output[single_len * i:single_len * (i + 1), Ellipsis]
    else:
        combined['actual'][df[0]] = output[25+single_len * i:single_len * (i + 1), Ellipsis]
        combined['predictp50'][df[0]]=model.model.predict(input[25+single_len*i:single_len*(i+1),Ellipsis])[Ellipsis,1]
        combined['predictp90'][df[0]]=model.model.predict(input[25+single_len*i:single_len*(i+1),Ellipsis])[Ellipsis,2]
        combined['predictp10'][df[0]]=model.model.predict(input[25+single_len*i:single_len*(i+1),Ellipsis])[Ellipsis,0]



  # for id,df  in enumerate(test.groupby('building_id')):
  #   combined=model.predict(df)
  # judge=(blist==b1list)
  return combined

def writecurrentbest(bestparam,his):
    with open("testWOA-MVMD.txt", 'w') as f:
        f.write("best param: {}\n".format(bestparam))
